...
class Table:
    def __init__(self, table_frame):
        ...
        self.tree.heading("name", text="Name",
                          command=lambda: self.sort(1, False))
        self.tree.heading("creation_date", text="Сreation date",
                          command=lambda: self.sort(2, False))
        self.tree.heading("last_modified", text="Last modified",
                          command=lambda: self.sort(3, False))
    ...
    def sort(self, col, reverse):
        l = sorted([(self.tree.set(k, col), k)
                   for k in self.tree.get_children("")], reverse=reverse)
    ...
    def refresh(self):
        ...
        notes = lib.DataManager.import_from_json()
